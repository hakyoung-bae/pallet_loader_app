...
if st.button("1단 적재 패턴 계산 및 시각화"):
    ...
    for idx, row in df.iterrows():
        ...
        for rect in rects:
            ...
            ax.text(x + rw/2, y + rh/2, rid, ha='center', va='center', fontsize=8, color='black')

        item_weight = qty * weight
        total_weight += item_weight

        st.write(f"모델명: **{model}**, 1단 적재수량: **{qty}개**, 단품 중량: **{weight}kg**, 총 중량: **{item_weight:.2f} kg**")
